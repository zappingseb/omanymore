...
source(here("R","test_googlesheets.R"))
source(here("R","kml_handling.R"))
source(here("R","switchButton.R"))

ui <- bootstrapPage(
  useShinyjs(),
  tags$style(type = "text/css", "html, body {width:100%;height:100%}"),
  tags$head(includeCSS(here("inst","css","button.css"))),
  fluidPage(
    fluidRow(
      ...
      column(12, div(style="width:100%;margin-left:10px;margin-right:10px;margin-top:10px",
                     uiOutput("table_container")))
      
    )
  ),
  absolutePanel(top = 10, right = 10,width="30%",fixed=T,style="z-index:10",
                fluidPage(
                  fluidRow(
                    column(4,
...
server <- function(input, output, session) {
 ...
 # sheets <- GoogleTable$new("https://docs.google.com/spreadsheets/d/16O1MSjRnD5epMqHIpKFQvp2o7jsrH1kYfBo3a2hj6T8/edit?usp=sharing")
  ...
  view_data <- read.csv(here("inst","oman_info.csv"),header=T,stringsAsFactors = F,na.strings = "")
  ...
  filteredData_by_table_upper_lower <- reactive({
    if(!is.null(input$table_select$select$r)){
      ...
      if(first_row_above==-Inf || first_row_behind==-Inf){
        NULL
      }else{
  ...
  output$table= 
    ...
    rhandsontable(data=view_data,
                  ...
                  readOnly = TRUE,
                  width="100%",
                  height="100%")%>%hot_table(stretchH = "all")%>%
              ...
              if(input$route_switch){
                output$leaflet_map <- renderLeaflet({
                  leaflet(data = routing_table,height="300px") %>% addTiles() %>% 
                    ...
                    clearShapes() %>%
                    addPolylines(data = routing_table, lng = ~long, lat = ~lat,label="high_route",color="#F00") %>%
                    addPolylines(data = map_data_reactive, lng = ~long, lat = ~lat)
                }) 
              }else{
                output$leaflet_map <- renderLeaflet({
